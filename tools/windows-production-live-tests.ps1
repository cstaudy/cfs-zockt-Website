...
function Secret([string]$n,[string]$prompt){$v=[Environment]::GetEnvironmentVariable($n,'Process');if(-not[string]::IsNullOrWhiteSpace($v)){Info "$n bereits lokal gesetzt; Wert wird nicht angezeigt.";return $v};$s=Read-Host "$prompt [SKIP = diesen Test überspringen]" -AsSecureString;$ptr=[Runtime.InteropServices.Marshal]::SecureStringToBSTR($s);try{$plain=[Runtime.InteropServices.Marshal]::PtrToStringBSTR($ptr);if($plain -ceq 'SKIP'){throw "Test vom Benutzer übersprungen: $n fehlt."};return $plain}finally{[Runtime.InteropServices.Marshal]::ZeroFreeBSTR($ptr)}}
function TextValue([string]$n,[string]$prompt,[string]$default=''){$v=[Environment]::GetEnvironmentVariable($n,'Process');if(-not[string]::IsNullOrWhiteSpace($v)){return $v};$label="$prompt [SKIP = diesen Test überspringen]";if($default){$label="$prompt [$default; SKIP = überspringen]"};$r=Read-Host $label;if($r -ceq 'SKIP'){throw "Test vom Benutzer übersprungen: $n fehlt."};if([string]::IsNullOrWhiteSpace($r)){return $default};return $r}
...
function EnsurePostgresClientTools(){
    $needed=@('pg_dump.exe','pg_restore.exe','psql.exe')
    $missing=@($needed|Where-Object{-not(Has $_)})
    if($missing.Count -eq 0){Ok 'PostgreSQL Clienttools im PATH gefunden.';return}
    $roots=@('C:\Program Files\PostgreSQL','C:\Program Files (x86)\PostgreSQL')
    $bins=@()
    foreach($base in $roots){
        if(Test-Path -LiteralPath $base){
            $bins+=@(Get-ChildItem -LiteralPath $base -Directory -ErrorAction SilentlyContinue | ForEach-Object {Join-Path $_.FullName 'bin'} | Where-Object {Test-Path -LiteralPath (Join-Path $_ 'pg_dump.exe')})
        }
    }
    $bin=$bins|Sort-Object -Descending|Select-Object -First 1
    if($null-ne$bin){$env:Path="$bin;$env:Path";Info "PostgreSQL Clienttools automatisch gefunden: $bin"}
    $missing=@($needed|Where-Object{-not(Has $_)})
    if($missing.Count -gt 0){throw ('R60 Voraussetzung fehlt: PostgreSQL Clienttools nicht gefunden: '+($missing -join ', ')+'. Installiere PostgreSQL Client Tools und starte ALL erneut.')}
    Ok 'PostgreSQL Clienttools bereit.'
}
...
function InvokeAllStep([string]$name,[scriptblock]$action,[hashtable]$results){
    ...
    try{
        ...
        $where=''
        if(-not[string]::IsNullOrWhiteSpace([string]$scriptName) -and $line){$where="$scriptName`:$line"}
        elseif($line){$where="Zeile $line"}
        $detail=if($where){"$message [$where]"}else{$message}
        $results[$name]=@{status='BLOCKED/FAIL';detail=$detail}
        Warn "$name BLOCKED/FAIL: $detail"
        return $false
    }
}
function RunAll([string]$root){
    ...
    Write-Host 'Dieser Lauf arbeitet R59-R66 nacheinander ab und hält nur bei echten externen/manuellen Nachweisen an.'`n    Write-Host 'Wenn eine externe Voraussetzung noch fehlt, bei der Eingabe SKIP tippen; ALL fährt dann mit dem nächsten unabhängigen Test fort.'
