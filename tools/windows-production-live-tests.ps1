...
function FreshClone(){if(-not(Has 'git')){throw 'git fehlt.'};$dest=Join-Path (Desktop) 'cfs-zockt-production-final-tests';if(Test-Path $dest){if(ProjectOk $dest){return $dest};$dest=Join-Path (Desktop) ('cfs-zockt-production-final-tests-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))};Info "Klone aktuellen GitHub-Stand nach $dest";& git clone --depth 1 $RepoUrl $dest;if($LASTEXITCODE-ne 0){throw "git clone fehlgeschlagen: $LASTEXITCODE"};if(-not(ProjectOk $dest)){throw 'GitHub-Stand enthält die R60-R67 Finalisierung noch nicht vollständig.'};return $dest}
function FindRoot(){foreach($p in @((Split-Path -Parent $PSScriptRoot),(Get-Location).Path,(Join-Path (Desktop) 'cfs-zockt-production-final-tests'),(Join-Path (Desktop) 'cfs-zockt-production-test'))){if(ProjectOk $p){return $p}};return FreshClone}
function UpdateRoot([string]$p){if($NoUpdate -or -not(Test-Path (Join-Path $p '.git'))){return $p};Push-Location $p;try{$dirty=@(& git status --porcelain);if($dirty.Count-eq 0){& git pull --ff-only;if($LASTEXITCODE-ne 0){throw 'git pull fehlgeschlagen.'};return $p}else{Warn 'Checkout hat lokale Änderungen; verwende für die Tests einen frischen GitHub-Checkout.'}}finally{Pop-Location};return FreshClone}
...
function Npm([string]$title,[string[]]$args){Section $title;& npm @args;if($LASTEXITCODE-ne 0){throw "$title fehlgeschlagen (ExitCode=$LASTEXITCODE)"};Ok $title}
function NodeStep([string]$title,[string[]]$args){Section $title;& node @args;if($LASTEXITCODE-ne 0){throw "$title fehlgeschlagen (ExitCode=$LASTEXITCODE)"};Ok $title}
...
function R59([string]$root){Npm 'R59 Security Gate' @('run','security59:check');Section 'R59 echter Production LIVE Drill';& npm run render:drill -- --external-only --live --target 'https://cfs-zockt.de';if($LASTEXITCODE-ne 0){throw "R59 BLOCKED/FAIL ($LASTEXITCODE)"};Harvest $root;Ok 'R59 LIVE_PASS'}
...
function Check([string]$root){foreach($n in 59..67){$s="security$n`:check";Npm "R$n statisches Gate" @('run',$s)};Ok 'R59-R67 statische Gates vollständig.'}
try{foreach($cmd in @('git','node','npm')){if(-not(Has $cmd)){throw "Benötigtes Programm fehlt: $cmd"}};$root=FindRoot;$root=UpdateRoot $root;Set-Location $root;Harvest $root;SyncIn $root;Section 'CFS ZOCKT Production LIVE Tests R59-R67';Write-Host "Repo: $root";Write-Host "Node: $(& node --version)";Npm 'npm ci' @('ci');if($Round-eq'MENU'){ShowEvidence;Write-Host '';Write-Host 'R59 Render | R60 Recovery | R61 Mail | R62 Passkey/MFA | R63 Windows | R64 2h OBS/LIVE | R65 Monitoring | R66 Stripe LIVE | R67 Launch Gate | CHECK';$Round=(Read-Host 'Welche Runde starten?').Trim().ToUpperInvariant()};switch($Round){'R59'{R59 $root};'R60'{R60 $root};'R61'{R61 $root};'R62'{R62 $root};'R63'{R63 $root};'R64'{R64 $root};'R65'{R65 $root};'R66'{R66 $root};'R67'{R67 $root};'CHECK'{Check $root};default{throw "Unbekannte Runde: $Round"}};Harvest $root;ShowEvidence;exit 0}catch{Write-Host '';Write-Host 'BLOCKED / FAIL';Write-Host $_.Exception.Message;exit 10}finally{RestoreEnv}
