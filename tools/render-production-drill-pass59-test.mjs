...
check('generated secret diagnostics name missing secrets',()=>assert.match(drill,/missingGenerated\.join\(', '\)/));
check('Blueprint generates account elevation secret',()=>assert.match(blueprint,/key:\s*CFS_ACCOUNT_ELEVATION_SECRET[\s\S]{0,80}generateValue:\s*true/));
...
check('HTTPS root retries transient request failures',()=>assert.match(drill,/requestWithRetry\(`\$\{expectedOrigin\}\/`,\{redirect:'follow'\},3\)/));
check('root retry is limited to transient network failures',()=>assert.match(drill,/transientRequestError\(error\)/));
check('root retry exhaustion remains a blocking failure',()=>assert.match(drill,/failed after 3 attempts/));
